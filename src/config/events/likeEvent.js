 ...
 * @description
 * BullMQ **Queue Events** listener for the "like" queue.
 * This module monitors and logs key lifecycle events (`completed`, `failed`, `waiting`, and `active`)
 * for jobs processed in the `like-Queue-Events` queue.
 *
 ...
 * @description
 * A BullMQ `QueueEvents` instance configured to listen for events emitted by the "like" queue.
 ...
 * The queue name `"like-Queue-Events"` should match the related worker and queue setup for consistent event handling.
 */
